...
//         array('title','1,255', '标题只能为1~250个字符', 1 , 'length'),
    ...
    public function getUniqueNum($where){
        return $this->field('content_id,unique_num')
                    ->where($where)
                    ->find();
    }
 ...
 /**
    ...
    public function saveExtData($data){
        ...
        if(!empty($fieldsetInfo)){
            ...
            if(!$expandModel->saveData($type,$fieldsetInfo,$data['content_id'])){ //2015-3-17  新增 $data['content_id']
                $this->error = $expandModel->getError();
                return false;
            }
        }
    ...
    public function getDescription(){
        ...
        $description=substr($content, 0,1000);
